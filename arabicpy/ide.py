...
class _NCCALCSIZE_PARAMS(ctypes.Structure):
    _fields_ = [("rgrc", ctypes.wintypes.RECT * 3), ("lppos", ctypes.c_void_p)]


def apply_native_rounded_corners(widget):
    """Request Windows 11's normal rounded corners for our frameless window."""
    if os.name != "nt":
        return
    try:
        handle = int(widget.winId())
        ...
        # Older Windows versions do not expose this DWM attribute.
        pass
...
<<<<<<< Updated upstream
        ...
        run_menu.setLayoutDirection(self.direction)
...
>>>>>>> Stashed changes
        ...
        self.os_builder_activity_button = QPushButton("◫", objectName="activityButton")
        self.os_builder_activity_button.setCheckable(True)
        self.os_builder_activity_button.setToolTip(self.t("OS Builder"))
        self.os_builder_activity_button.clicked.connect(self.show_os_builder_page)
        self.activity_buttons.append(self.os_builder_activity_button)
        activity_layout.addWidget(self.os_builder_activity_button)
...
>>>>>>> Stashed changes
    ...
    def nativeEvent(self, eventType, message):
        # Windows still reserves its invisible resize-drag border for a
        # frameless-but-resizable window even with no titlebar, and when
        # maximized that reservation isn't clipped away -- it shows up as a
        # thin sliver of desktop along one edge. Shrinking the proposed
        # client rect by that border's own thickness during WM_NCCALCSIZE
        # is the standard fix (mirrors what Windows does for normal windows).
        if os.name == "nt" and eventType == b"windows_generic_MSG" and self.isMaximized():
            msg = ctypes.wintypes.MSG.from_address(int(message))
            if msg.message == WM_NCCALCSIZE and msg.wParam:
                params = _NCCALCSIZE_PARAMS.from_address(msg.lParam)
                rect = params.rgrc[0]
                border_x = ctypes.windll.user32.GetSystemMetrics(SM_CXSIZEFRAME) + ctypes.windll.user32.GetSystemMetrics(SM_CXPADDEDBORDER)
                border_y = ctypes.windll.user32.GetSystemMetrics(SM_CYSIZEFRAME) + ctypes.windll.user32.GetSystemMetrics(SM_CXPADDEDBORDER)
                rect.left += border_x
                rect.right -= border_x
                rect.top += border_y
...
>>>>>>> Stashed changes
        ...
        self.output.setPlainText(
            self.t("Cross-platform project exported to:\n{directory}\n\n"
                   "This includes project files for Browser, Windows, Linux, macOS, and iOS. "
