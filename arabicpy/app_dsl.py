"""Parser for الباء's declarative "app" (تطبيق) UI syntax.

Shared by the UI and export targets that turn this syntax into a real app.
"""
...
def parse_app_source(source):
    ...
    while index < len(lines):
        ...
        if function_definition:
            ...
            if button is None:
                ...
                if event is not None:
                    ...
                    event = AppEvent(button, function_name=function_name)
            else:
                event = AppEvent(button, function_name=function_name)
            index += 1
            while index < len(lines):
        ...
        if chat_match:
            ...
            last_widget = AppWidget(name, "دردشة", "", page=current_page)
        ...
        if natural_button_match:
            ...
            widget_names.add(name)
            last_widget = AppWidget(name, "زر", text, page=current_page)
        ...
        raise ArabicPyError(f"تعليمة تطبيق غير معروفة: {stripped}", line_number, 1)
    ...
    return AppProgram(title, widgets, events, background_color, bottom_navigation)
